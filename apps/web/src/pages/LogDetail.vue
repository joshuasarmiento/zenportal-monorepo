...
import { AlertOctagon, Clock, FileText, ExternalLink, ArrowLeft } from 'lucide-vue-next'
    ...
    <SidebarInset>
        ...
        <div class="flex items-center gap-2">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbPage>Log Details</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
      </header>
      ...
      <div class="flex flex-1 flex-col p-4 md:p-8 bg-muted/40 overflow-y-auto">
        ...
        <div v-else-if="log" class="max-w-3xl mx-auto space-y-6 w-full">
          ...
          <div v-if="log.isBlocked" class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900 rounded-xl p-6 flex items-start gap-4">
          ...
          <Card class="overflow-hidden">
            <div class="bg-muted/30 px-6 py-4 border-b border-border flex justify-between items-center">
              <Badge variant="secondary" class="bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300">
            ...
            <CardContent class="p-8 space-y-6">
              <div>
                ...
                <p class="text-muted-foreground whitespace-pre-line leading-relaxed">{{ log.summary }}</p>
              </div>

              <div v-if="log.attachmentUrl">
                <h3 class="text-foreground font-bold text-sm mb-2 uppercase tracking-wide">Attachments</h3>
                <a :href="log.attachmentUrl" target="_blank" class="flex items-center gap-3 p-4 border border-border rounded-lg hover:bg-muted/50 transition group">
                    <div class="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 p-2 rounded">
                      <FileText class="h-5 w-5" />
                    </div>
                    <div>
                      <p class="font-medium text-foreground group-hover:text-primary transition">View Attached File</p>
                      <p class="text-xs text-muted-foreground truncate max-w-sm">{{ log.attachmentUrl }}</p>
                    </div>
                    <ExternalLink class="h-4 w-4 text-muted-foreground ml-auto group-hover:text-primary" />
                </a>
              </div>
            </CardContent>
          </Card>
