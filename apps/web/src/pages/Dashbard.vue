...
<template>
  <SidebarProvider>
    <AppSidebar />
    <SidebarInset>
      <header class="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-12 border-b border-gray-200 bg-white px-4 sticky top-0 z-10">
      ...
      <div class="flex flex-1 flex-col gap-4 p-4 pt-4 bg-slate-50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card>
                <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <Briefcase class="h-4 w-4 text-green-600" />
                </CardHeader>
                <CardContent>
            ...
            <Card>
                ...
                <CardContent>
                    <div class="text-2xl font-bold text-red-600">{{ stats.pendingBlockers }}</div>
                </CardContent>
            </Card>
        </div>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            <TableRow 
                                ...
                                :class="{ 'bg-red-50/50 hover:bg-red-50': log.isBlocked }"
                            >
                                <TableCell class="font-medium text-muted-foreground">{{ log.date }}</TableCell>
                                <TableCell>
                                    <Badge variant="secondary" class="bg-indigo-100 text-indigo-700 hover:bg-indigo-100">
                                ...
                                <TableCell>
                                    <div class="flex flex-col gap-1 items-start">
                                        <div v-if="log.videoUrl" class="flex items-center gap-1.5 text-blue-600 text-xs font-semibold">
                                            <Video class="h-3.5 w-3.5" /> Loom
                                        </div>
                                        <div v-if="log.attachmentUrl" class="flex items-center gap-1.5 text-indigo-600 text-xs font-semibold">
                                ...
                                <TableCell>
                                    ...
                                    <Badge v-else variant="outline" class="gap-1 border-green-200 bg-green-50 text-green-700">
