  ...
  <div class="max-w-2xl mx-auto p-8">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <form @submit.prevent="submit" class="p-6 space-y-6">
        ...
        <div>
          <label class="block text-sm font-semibold text-gray-700 mb-2">What did you accomplish?</label>
          <textarea v-model="form.summary" rows="4" class="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500"></textarea>
        </div>

        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Loom Link</label>
        ...
        <div class="bg-red-50 rounded-lg p-4 border border-red-100">
          ...
          <div v-if="form.isBlocked" class="mt-3">
             <input v-model="form.blockerDetails" type="text" class="w-full p-2 bg-white border border-red-200 rounded text-sm" placeholder="What do you need from the client?">
          </div>
        </div>
