...
import { Search, Plus, Loader2, Link as LinkIcon, Pencil, UserPlus } from 'lucide-vue-next'
    ...
    <SidebarInset>
      <header class="flex h-16 shrink-0 items-center gap-2 border-b border-border bg-background px-4 sticky top-0 z-10">
        ...
        <div class="ml-auto">
          <Button @click="goToAdd"
            class="gap-2 bg-slate-900 hover:bg-slate-800 dark:bg-slate-50 dark:text-slate-900 dark:hover:bg-slate-200">
      ...
      <div class="flex flex-1 flex-col p-4 md:p-8 bg-muted/40 overflow-y-auto">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <Card v-for="(c, index) in processedClients" :key="c.id"
            ...
            <CardHeader class="flex flex-row items-start justify-between space-y-0 pb-2">
              <div class="w-12 h-12 rounded-lg flex items-center justify-center font-bold text-lg"
                :class="index % 2 === 0 ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/50 dark:text-indigo-300' : 'bg-orange-100 text-orange-600 dark:bg-orange-900/50 dark:text-orange-300'">
                {{ c.companyName.substring(0, 2).toUpperCase() }}
              </div>
              <Badge :variant="c.status === 'archived' ? 'secondary' : 'default'" class="uppercase">{{ c.status ||
                'Active' }}</Badge>
            </CardHeader>
          ...
          <button @click="goToAdd"
            class="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl p-6 flex flex-col items-center justify-center text-muted-foreground hover:border-primary hover:text-primary hover:bg-muted/50 transition cursor-pointer min-h-[280px] group bg-background">
            <div
              class="w-14 h-14 bg-muted rounded-full flex items-center justify-center mb-3 group-hover:bg-primary/10 transition-colors">
              <Plus class="h-6 w-6" />
            </div>
